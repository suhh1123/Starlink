...
const WorldMap = ({
  ...
  const updateMarker = (data, idx) => {
    setMarkersInfo(data.map((sat, index) => {
      coord[index].push([sat.positions[idx].satlongitude, sat.positions[idx].satlatitude]);
      return {
        lon: sat.positions[idx].satlongitude,
        lat: sat.positions[idx].satlatitude,
  ...
  const coord = []; // use to keep track of the position of each seleted satellite
  const trackOnClick = () => {
